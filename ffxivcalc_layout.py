...
# This part of the code will execute whatever rotation is written here. It will
# be called from TUI.

Dummy = Enemy()
Event = Fight(Enemy=Dummy, ShowGraph=True, Level=90)
...
# Stat Sheet
# Enter your own stats here. The default is the 6.2 Savage BiS found on the
# Balance. The default Tenacity for non-tank is 400.
...
GNBStat = {
    ...
    "Crit": 2352,
    "DH": 868,
}  # Stats for Gunbreaker

# ===============================================================================================

# Here the player objects are being initialized. You do not need to change
# anything here.
# Note that if you want to simulate with two players of the same Jobs you will
# need to create another Player Object here.
...
# Here you can enter the action list you want the simulator to simulate. If you
# want to simulate a BlackMage go to the respective list, in that case you would
# write in BLMOpener\
# Note that this action list also includes the prepull. The simulator will start
# at soon as one of the player character does damage. So coordinate your different
# player with WaitAbility() so they all start at the same time.
# Note that if you are simulating with more than 1 per job you will need to create a new list of actions.

# Caster
BLMOpener = [Fire3, Fire4, Fire4, Fire4, Fire4, Fire4, Despair]
SMNOpener = []
RDMOpener = [
    Verthunder,
    Verareo,
...
# Here we are linking the earlier created action list to the player object. You
# should not have to change anything here except if you are trying to simulate
# with more than 1 player per job. In which case you will need to link the earlier
# created object and the earlier created action list.
...
# Time limit for the simulation in seconds. It will stop once this time has been
# reached (in simulation time)
TimeLimit = 500
# Time unit or frame of the simulation. Smallest step the simulator will take
# between each iterations. It is advised to not change this value
time_unit = 0.01
# Parameter to show (or not) the graph generated by the simulator.
ShowGraph = True
# Parameter that will enable or disable the requirement check for all actions. If
# False the simulator will not check if an action can be done
RequirementOn = True
