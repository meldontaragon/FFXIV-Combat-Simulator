...
def ApplyDoomSpike(Player, Enemy):
    if not (DoomSpikeCombo in Player.EffectList):
        Player.EffectList.append(DoomSpikeCombo)
...
def ApplyTrueThrust(Player, Enemy):
    if not (TrueThrustCombo in Player.EffectList):
        Player.EffectList.append(TrueThrustCombo)
...
def BattleLitanyCheck(Player, Enemy):
    if Player.BattleLitanyTimer <= 0:
        for player in Player.CurrentFight.PlayerList:
            player.CritRateBonus -= 0.1  # Removing buff
...
DraconianFury = DragoonSpell(
    25770, True, 2.5, 130, ApplyRaidenThrust, [RaidenThrustRequirement], True
)  # AOE version of Raiden Thrust
