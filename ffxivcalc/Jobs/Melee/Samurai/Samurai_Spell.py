...
def ApplyFuko(Player, Enemy):
    AddKenki(Player, 10)
    if not (FukoCombo in Player.EffectList):
        Player.EffectList.append(FukoCombo)
...
def ApplyJinpu(Player, Enemy):
    if not (JinpuEffect in Player.EffectList):
        Player.EffectList.append(JinpuEffect)


def ApplyShifu(Player, Enemy):
    if not (ShifuEffect in Player.EffectList):
        Player.EffectList.append(ShifuEffect)
...
def ApplyMidare(Player, Enemy):
    ...
    Player.GuarenteedCrit = True
    Player.Setsu, Player.Ka, Player.Getsu = False, False, False  # Reseting


def ApplyKaeshiSetsugekka(Player, Enemy):
    Player.GuarenteedCrit = True
...
def HakazeEffect(Player, Spell):
    if Spell.id == Jinpu.id:
        ...
        Spell.Potency += 160
        if not (FukaEffect in Player.EffectList):
            Player.EffectList.append(FukaEffect)
...
def TsubamegaeshiCheck(Player, Spell):
    ...
    if Spell.GCD:
        if Player.KaeshiHiganbana:
            if Spell.id != KaeshiHiganbana.id:
                Player.KaeshiHiganbana = False
        elif Player.KaeshiGoken:
            if Spell.id != KaeshiGoken.id:
                Player.KaeshiGoken = False
        elif Player.KaeshiSetsugekka:
            if Spell.id != KaeshiSetsugekka.id:
                Player.KaeshiSetsugekka = False
...
# Kaeshi
KaeshiHiganbana = SamuraiSpell(
    16484, True, 0, 2.5, 200, ApplyKaeshiHiganbana, [KaeshiHiganbanaRequirement, TsubamegaeshiRequirement], 0
)
KaeshiGoken = SamuraiSpell(
    16485, True, 0, 2.5, 280, ApplyKaeshiGoken, [KaeshiGokenRequirement, TsubamegaeshiRequirement], 0
)
KaeshiSetsugekka = SamuraiSpell(
    16486, True, 0, 2.5, 640, ApplyKaeshiSetsugekka, [KaeshiSetsugekkaRequirement, TsubamegaeshiRequirement], 0
)
