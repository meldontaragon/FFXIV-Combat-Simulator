...
def ApplySpinningScythe(Player, Enemy):
    Player.AddGauge(10)
    if not (SpinningScytheCombo in Player.EffectList):
        Player.EffectList.append(SpinningScytheCombo)
...
def ApplySlice(Player, Enemy):
    if not (SliceCombo in Player.EffectList):
        Player.EffectList.append(SliceCombo)
...
# Other GCD
Soulsow = ReaperSpell(24387, True, 5, 2.5, 0, ApplySoulsow, [], False, SpeedEnum.SpS)
HarvestMoon = ReaperSpell(24388, True, Lock, 2.5, 600, ApplyHarvestMoon, [HarvestMoonRequirement], False, SpeedEnum.SpS)
Harpe = ReaperSpell(24386, True, 1.3, 2.5, 300, empty, [], False, SpeedEnum.SpS)
...
Gallows = ReaperSpell(
    24383, True, Lock, 2.5, 460, ApplyGallows, [GibbetRequirement], True
)  # shares same requirement as Gibbet
...
CrossReaping = ReaperSpell(
    24396, True, Lock, 1.5, 460, ApplyCrossReaping, [VoidReapingRequirement], True
)  # Same Requriement as VoidReaping
Communio = ReaperSpell(24398, True, 1.3, 2.5, 1100, ApplyCommunio, [CommunioRequirement], False, SpeedEnum.SpS)
...
UnveiledGallows = ReaperSpell(
    24391, False, Lock, 0, 400, ApplyUnveiledGibbet, [UnveiledGallowsRequirement], False
)  # Shares effect with Gibbet
...
Guillotine = ReaperSpell(
    24384, False, 0, 0, 200, ApplyGuillotine, [GibbetRequirement], True
)  # AOE version of Gallows/Gibbet
# AOE oGCD
GrimSwath = ReaperSpell(
    24392, False, 0, 0, 140, ApplyBloodStalk, [BloodStalkRequirement], False
)  # AOE version of bloodstalk
SoulScyte = ReaperSpell(
    24381, False, 0, 0, 180, ApplySoulSlice, [SoulSliceRequirement], False
)  # AOE version of SoulSlice
GrimReaping = ReaperSpell(24397, True, 0, 2.5, 200, ApplyGrimReaping, [VoidReapingRequirement], False)
LemureScythe = ReaperSpell(
    24400, False, 0, 0, 100, ApplyLemureSlice, [LemureSliceRequirement], False
)  # AOE version of Lemure Slice
# buff
