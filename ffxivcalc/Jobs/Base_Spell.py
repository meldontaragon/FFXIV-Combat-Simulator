...
import math
from ffxivcalc.Jobs.PlayerEnum import JobEnum, RoleEnum, SpeedEnum
from ffxivcalc.helperCode.requirementHandler import failedRequirementEvent

Lock = 0.75  # TODO: look into whether this value is too high
...
class Spell:
    ...
    def Cast(self, player, Enemy):
        """
        This function is called by the simulator when an action is ready to begin
        its casting. It checks for the requirement and apply all effect
        currently in the fight that can affect the action. It will lock the player
        into casting mode if necessary. The action is not executed here
        and is in a way being preapred to be executed. It will be checked if the
        action can be done.
        ...
        for Requirement in tempSpell.Requirement:
            ...
            # Requirements return both whether it can be casted and will take away
            # whatever value needs to be reduced to cast
            if not ableToCast:
                # Will check if timeLeft is within a margin, so we will just wait
                # for it to come timeLeft is the remaining time before the spell is available

                addInfo = (
                    ""
                    if timeLeft <= 0
                    else "player had to wait for or would have to wait for " + str(timeLeft) + " seconds."
                )

                fatal = not (timeLeft <= player.CurrentFight.waitingThreshold and timeLeft > 0) and (
                    player.CurrentFight.RequirementOn
                )  # true if stops the simulation

                # Recording the event~
                newFailedRequirementEvent = failedRequirementEvent(
                    player.CurrentFight.TimeStamp, player.playerID, Requirement.__name__, addInfo, fatal
                )
                # storing the event in memory
                player.CurrentFight.failedRequirementList.append(newFailedRequirementEvent)

                if not (player.CurrentFight.RequirementOn):
                    # If we do not care about requirement simply go on.
                    return tempSpell
                # If we care about requirement, we check if we can wait the allocated threshold. if we can we wait for it to come off cooldown.
                elif timeLeft <= player.CurrentFight.waitingThreshold and timeLeft > 0:
        ...
        if tempSpell.id > 0 and tempSpell.CastTime < Lock:
            # id < 0 are special abilities like DOT, so we do not want them to be affected by that
            tempSpell.CastTime = 0
        return tempSpell
    ...
    def CastFinal(self, player, Enemy):
        ...
        if self.Potency != 0:
            minDamage, Damage = player.CurrentFight.ComputeDamageFunction(
                player, self.Potency, Enemy, self.DPSBonus, type, self
            )  # Damage computation
        else:
            minDamage, Damage = 0, 0
        ...
        if not (player.CurrentFight.FightStart) and Damage > 0:
            ...
            for gamer in player.CurrentFight.PlayerList:
                if gamer.JobEnum == JobEnum.Monk:
                    gamer.DOTList.append(copy.deepcopy(Monk_Auto))
                elif (
                    gamer.RoleEnum == RoleEnum.Melee
                    or gamer.JobEnum == JobEnum.Dancer
                    or gamer.RoleEnum == RoleEnum.Tank
                ):
        ...
        if not (isinstance(self, DOTSpell)):
            player.NextSpell += 1  # Only increase counter if action was not a DOT
        ...
        if self.GCD:
            player.GCDCounter += 1  # If action was a GCD, increase the counter
...
def ManaRequirement(player, Spell):
    ...
    if player.Mana >= Spell.ManaCost:
        # ManaRequirement is the only Requirement that actually removes Ressources
        player.Mana -= Spell.ManaCost
        return True, -1
    # Ignore mana is a field of the fight set to true if we ignore the mana
    return player.CurrentFight.IgnoreMana, -1
...
def WaitAbility(time: float):
    ...
    WaitAction = Spell(212, False, time, time, 0, 0, ApplyWaitAbility, [], SpeedEnum.NA)
...
def ApplyPotion(Player, Enemy):
    ...
    """
    Player.Stat["MainStat"] = min(
        math.floor(Player.Stat["MainStat"] * 1.1), Player.Stat["MainStat"] + 223
    )  # Grade 7 HQ tincture
...
class DOTSpell(Spell):
    ...
    def __init__(self, id, Potency, isPhysical, ssType: SpeedEnum = SpeedEnum.NA):
        """
        id : int -> id of the dot. Dot have negative ids
        Potency : int -> base potency of the DOT
        isPhysical : bool -> True if the dot is physical
        """
        super().__init__(id, False, 0, 0, Potency, 0, empty, [], ssType)
        # Note that here Potency is the potency of the dot, not of the ability
        # This represents the timer of the dot, and it will apply at each 3 seconds
        self.DOTTimer = 0
        ...
        self.DHBonus = 0
        self.MultBonus = []
        # This flag will be set to True once the DOT damage has been through damage computation once
...
class Auto_Attack(DOTSpell):
    ...
    def __init__(self, id, Ranged: bool):
        ...
        """
        if Ranged:
            super().__init__(id, 100, True)  # Ranged AA
        else:
            super().__init__(id, 110, True)  # Melee AA
...
Melee_AA = Spell(-30, False, 0, 0, 0, 0, ApplyMelee_AA, [], SSType=SpeedEnum.SkS)
Ranged_AA = Spell(-30, False, 0, 0, 0, 0, ApplyRanged_AA, [], SSType=SpeedEnum.SkS)
Queen_AA = Spell(-30, False, 0, 0, 0, 0, ApplyQueen_AA, [], SSType=SpeedEnum.SkS)

Monk_Auto = Monk_AA()
Melee_AADOT = Melee_Auto(-22, False)
Ranged_AADOT = Ranged_Auto(-23, True)
Queen_AADOT = Queen_Auto(-24, False)
Potion = Spell(-2, False, 1, 1, 0, 0, ApplyPotion, [], SpeedEnum.NA)
