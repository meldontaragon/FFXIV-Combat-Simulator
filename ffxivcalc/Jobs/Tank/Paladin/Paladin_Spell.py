...
def ApplyTotalEclipse(Player, Enemy):
    if not (TotalEclipseCombo in Player.EffectList):
        Player.EffectList.append(TotalEclipseCombo)
...
Confetti = PaladinSpell(
    16459, True, Lock, 2.5, 1000, 1000, ApplyConfetti, [ManaRequirement, ConfettiRequirement], False, SpeedEnum.SpS
)  # >.>
BladeFaith = PaladinSpell(
    25748, True, Lock, 2.5, 480, 0, ApplyBladeFaith, [BladeFaithRequirement], False, SpeedEnum.SpS
)
BladeTruth = PaladinSpell(
    25749, True, Lock, 2.5, 560, 0, ApplyBladeTruth, [BladeTruthRequirement], False, SpeedEnum.SpS
)
BladeValor = PaladinSpell(
    25750, True, Lock, 2.5, 620, 0, ApplyBladeValor, [BladeValorRequirement], False, SpeedEnum.SpS
)
BladeValorDOT = DOTSpell(-11, 80, True)

# GCD
HolySpirit = PaladinSpell(7384, True, 1.5, 2.5, 300, 1000, empty, [ManaRequirement], False, SpeedEnum.SpS)
Atonement = PaladinSpell(16460, True, Lock, 2.5, 420, 0, ApplyAtonement, [AtonementRequirement], True)
Clemency = PaladinSpell(3541, True, 1.5, 2.5, 0, 1000, empty, [ManaRequirement], False, SpeedEnum.SpS)
ShieldLob = PaladinSpell(24, True, 0, 2.5, 100, 0, empty, [], True)
# AOE GCD
HolyCircle = PaladinSpell(16458, True, 1.5, 2.5, 130, 1000, empty, [ManaRequirement], False, SpeedEnum.SpS)
...
# oGCD
RequestACat = PaladinSpell(
    7383, False, 0, Lock, 400, 0, ApplyRequestACat, [RequestACatRequirement], True
)  # I NEED ONE RIGHT NOW :x
...
HolySheltron = PaladinSpell(
    25746, False, 0, 0, 0, 0, ApplyHolySheltron, [HolySheltronRequirement, SheltronRequirement], False
)
...
Intervention = PaladinSpell(
    7382, False, 0, 0, 0, 0, ApplyIntervention, [SheltronRequirement, InterventionRequirement], False
)
