...
def ApplyThunder4(Player, Enemy):
    ...
    if Player.Thunder3DOT != None:
        Player.Thunder3DOTTimer = 0
...
def ApplyTranspose(Player, Spell):
    ...
    if (Player.ElementalGauge == 3) or (Player.ElementalGauge == -3 and Player.UmbralHearts == 3):
        Player.Paradox = True  # In fire phase with 3 Astral Fire

    if Player.ElementalGauge > 0:
        Player.ElementalGauge = -1  # Ice
    elif Player.ElementalGauge < 0:
        Player.ElementalGauge = 1  # Fire
...
def TripleCastEffect(Player, Spell):
    # If GCD and not already insta cast, also Swift will go before
    if not (SwiftcastEffect in Player.EffectList) and Spell.GCD and Spell.CastTime > Lock:
        Spell.CastTime = Lock
...
def ElementalEffect(Player, Spell):
    ...
    if isinstance(Spell, BLMSpell) and Spell.IsFire:
        ...
        if Player.ElementalGauge > 0:  # Fire Phase
            ...
            # If we have UmbralHearts, then no mana cost increase. We also have to make sure
            if (Player.UmbralHearts > 0) and Spell.id != Flare.id:
                # it isn't Flare, since Flare has its own way of dealing with UmbrealHearts
                Player.UmbralHearts -= 1
            else:
                if Spell.id != Despair.id and Spell.id != Flare.id:
                    Spell.ManaCost *= 2  # Double mana cost, only if not despair
        # Check if ice phase
...
Fire4 = BLMSpell(3577, True, 2.8, 2.5, 310, 800, True, False, empty, [
                 EnochianRequirement, FireRequirement, ManaRequirement])  # BIG PP DAMAGE LETS GOOOOOOOOOOOOOo
...
Blizzard4 = BLMSpell(3576, True, 2.5, 2.5, 310, 800, False, True, ApplyBlizzard4,
                     [EnochianRequirement, IceRequirement, ManaRequirement])
Freeze = BLMSpell(159, True, 2.8, 2.5, 120, 1000, False, True, ApplyBlizzard4, [
                  EnochianRequirement, IceRequirement, ManaRequirement])  # Same as B4
...
Foul = BLMSpell(7422, True, Lock, 2.5, 600, 0, False, False, ApplyXenoglossy,
                [PolyglotRequirement])  # Same effect as Xeno
