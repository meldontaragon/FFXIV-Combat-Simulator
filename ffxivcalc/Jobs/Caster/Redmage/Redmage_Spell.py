...
def RDMManaRequirement(Player, Spell):
    return (
        Spell.ManaCost <= Player.Mana and Spell.BlackCost <= Player.BlackMana and Spell.WhiteCost <= Player.WhiteMana,
        -1,
    )
...
def ApplyEmbolden(Player, Enemy):
    Enemy.buffList.append(
        EmboldenBuff
    )  # 5% DPS boost for everyone. I put buff on Boss, but could put it on every other player
...
def ApplyRiposte(Player, Enemy):
    if not (Riposte in Player.EffectList):
        Player.EffectList.append(RiposteCombo)
...
def ApplyEnchantedRiposte(Player, Enemy):
    ...
    if not (Riposte in Player.EffectList):
        Player.EffectList.append(RiposteCombo)
    if not (ManaStackEffect in Player.EffectList):
        Player.EffectList.append(ManaStackEffect)
...
def ApplyVerholy(Player, Enemy):
    ...
    if Player.WhiteMana < Player.BlackMana:
        ...
        Player.ExpectedVerstoneProc += 0.2
...
def ApplyVerflare(Player, Enemy):
    ...
    if Player.WhiteMana > Player.BlackMana:
        ...
        Player.ExpectedVerfireProc += 0.2
...
def ManaStackEffect(Player, Spell):
    ...
    elif Spell.GCD and not (
        Spell.id == EnchantedRiposte.id or Spell.id == EnchantedZwerchhau.id or Spell.id == EnchantedRedoublement.id
    ):
...
def DualCastEffect(Player, Spell):
    if (
        Spell.CastTime != 0 and Spell.GCD and Spell.id != -1
    ):  # Want to make sure the spell will affect Dualcast, id != -1 is to make sure this is not WaitAbility
        if Player.DualCast:
            Spell.CastTime = 0  # Insta cast half of the spells, will be put by default for RedMage
            Player.DualCast = False  # Remove Dualcast
        else:
            Player.DualCast = True  # Give Dual cast
...
def AccelerationEffect(Player, Spell):
    if (Spell.id == Verthunder.id or Spell.id == Verareo.id or Spell.id == Impact.id) and not (
        SwiftcastEffect in Player.EffectList
    ):
        Spell.CastTime = 0
        Player.EffectToRemove.append(AccelerationEffect)
        if Spell.id == Impact.id:
            Spell.Potency += 50  # Impact has high potency when used with Acceleration
...
# Combo actions
# NonEnchanted
Riposte = RedmageSpell(7504, True, 0, 2.5, 130, 0, ApplyRiposte, [], 0, 0, SpeedEnum.SkS)
Zwerchhau = RedmageSpell(7512, True, 0, 2.5, 100, 0, empty, [], 0, 0, SpeedEnum.SkS)
Redoublement = RedmageSpell(7516, True, 0, 2.5, 100, 0, empty, [], 0, 0, SpeedEnum.SkS)
Reprise = RedmageSpell(16529, True, 0, 2.5, 100, 0, empty, [], 0, 0, SpeedEnum.SkS)
# Enchanted
EnchantedRiposte = RedmageSpell(
    7527, True, 0, 1.5, 280, 0, ApplyEnchantedRiposte, [RDMManaRequirement], 20, 20, SpeedEnum.SkS
)
EnchantedZwerchhau = RedmageSpell(
    7528, True, 0, 1.5, 150, 0, ApplyZwerchhau, [RDMManaRequirement], 15, 15, SpeedEnum.SkS
)
EnchantedRedoublement = RedmageSpell(
    7529, True, 0, 2.2, 130, 0, ApplyRedoublement, [RDMManaRequirement], 15, 15, SpeedEnum.SkS
)
EnchantedReprise = RedmageSpell(16528, True, 0, 2.5, 330, 0, empty, [RDMManaRequirement], 5, 5, SpeedEnum.SkS)

Verholy = RedmageSpell(7526, True, 0, 2.5, 580, 400, ApplyVerholy, [ManaRequirement, VerholyRequirement], 0, 0)
Verflare = RedmageSpell(
    7525, True, 0, 2.5, 580, 400, ApplyVerflare, [ManaRequirement, VerholyRequirement], 0, 0
)  # Same Requirement as Verholy, just need 3 Mana stacks
Scorch = RedmageSpell(16530, True, 0, 2.5, 680, 400, ApplyScorch, [ManaRequirement, ScorchRequirement], 0, 0)
Resolution = RedmageSpell(
    25858, True, 0, 2.5, 750, 400, ApplyResolution, [ManaRequirement, ResolutionRequirement], 0, 0
)
# AOE Melee Action
Moulinet = RedmageSpell(7513, True, 0, 1.5, 130, 0, ApplyMoulinet, [RDMManaRequirement], 20, 20, SpeedEnum.SkS)
EnchantedMoulinet = RedmageSpell(
    30, True, 0, 1.5, 130, 0, ApplyEnchantedMoulinet, [RDMManaRequirement], 20, 20, SpeedEnum.SkS
)
